...
export async function addEntry (backendMap:BackendMap, left: string, right: string): Promise<void> {
  const key = await backendMap.createKey();
  await backendMap.set(
    key,
    JSON.stringify(new EntryModel(key, EntryStatus.DELETED, "").setLeft(left).setRight(right).data)
  );
}
